...
const listProxy = [
    { path: '/vless', proxy: '172.232.238.169' },
    { path: '/vless', proxy: '172.232.238.169' },
    //tambahin sendiri
];
let proxyIP;
...
async function getAllConfigVless(hostName) {
    try {
        ...
        for (const entry of listProxy) {
            ...
            const vlessTls = `vless://3b862066-6fb9-4db1-a96a-c92beecaaa15\u0040${hostName}:443?encryption=none&security=tls&sni=${hostName}&fp=randomized&type=ws&host=${hostName}&path=${pathFixed}#${data.isp} (${data.country_code})`;
            const vlessNtls = `vless://3b862066-6fb9-4db1-a96a-c92beecaaa15\u0040${hostName}:80?path=${pathFixed}&security=none&encryption=none&host=${hostName}&fp=randomized&type=ws&sni=${hostName}#${data.isp} (${data.country_code})`;
